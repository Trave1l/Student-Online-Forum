<template>
  ...
  <el-card style="width: 96%; margin: 20px; text-align: center; background: rgba(0,0,0,0)">
    ...
    <div>
      ...
      <el-button @click="StoreBbs" type="text" style="font-size: large; margin-left: 20px">Store</el-button>
    </div>
    <br />
...
export default {
  ...
  methods:{
    ...
    StoreBbs(){
      ...
      request.get("/api/store/bbsUID/" + user.id).then(res=>{
    ...
    Jump(row){
      ...
      sessionStorage.setItem("BbsId", row.bbsId)
      this.$router.push('/details')
      request.put("/api/bbs/", row)
    },
    handleDelete(id){
      request.delete("/api/store/" + id).then(res =>{
        if(res.code == '0'){
          this.$message({
            type:"success",
            message:"删除成功"
